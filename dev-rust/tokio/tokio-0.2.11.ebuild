...
LICENSE="MIT"
KEYWORDS="~amd64 ~x86"
IUSE="fs full io-driver io-util macros net parking-lot process rt-threaded signal stream sync tcp test time udp uds"
...
F_PARKING_LOT="=dev-rust/parking_lot-0.10*:="
...
F_TOKIO_MACROS="( =dev-rust/tokio-macros-0.2*:= >=dev-rust/tokio-macros-0.2.4 )"
...
F_SYNC="${F_FNV}"
F_TIME="${F_SLAB}"
F_FULL="${F_FS} ${F_IO_DRIVER} ${F_IO_UTIL} ${F_MACROS} ${F_NET}
	${F_RT_THREADED} ${F_SIGNAL} ${F_STREAM} ${F_SYNC} ${F_TIME}"

DEV_DEPEND="
	=dev-rust/futures-0.3*:=[async-await,executor,std]
	( =dev-rust/loom-0.2*:=[checkpoint,futures] >=dev-rust/loom-0.2.13 )
	...
	${F_PARKING_LOT} ${F_SIGNAL_HOOK_REGISTRY}
	${F_SLAB} ${F_TOKIO_MACROS}
"
BDEPEND="
	=dev-rust/bytes-0.5*:=
...
"
PATCHES=(
	"${FILESDIR}/${PN}-0.2.9-unix-cargo.patch"
)
src_test() {
