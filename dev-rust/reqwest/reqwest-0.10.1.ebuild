...
IUSE="blocking cookies +default-tls gzip json native-tls native-tls-vendored rustls-tls socks"
F_BLOCKING="
...
src_test() {
	local targets=(
		...
		# broken, cannot infer type
		# '__tls'
		# broken test/blocking test_blocking_inside_a_runtime
		# 'blocking'
		'cookies'
		'default'
		'default-tls'
	...
	# NB: Incremental compilation leads to a lot of disk usage
	# which could easily OOD on this crate.
	for i in "${targets[@]}"; do
		einfo "Testing --features '${i}'"
		RUSTFLAGS="${RUSTFLAGS} --cap-lints warn" RUST_BACKTRACE=1 CARGO_INCREMENTAL=0 ecargo test --no-default-features --features "${i}"
	done
	einfo "Testing (synthetic) --all-features"
	RUSTFLAGS="${RUSTFLAGS} --cap-lints warn"  RUST_BACKTRACE=1 CARGO_INCREMENTAL=0 ecargo test --no-default-features --features "${targets[*]}"
}
